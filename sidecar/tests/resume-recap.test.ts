...
test("maybeSendResumeRecap stays quiet for recent or in-flight sessions", async () => {
	...
	expect(await maybeSendResumeRecap(stubSession, recentSession, NOW)).toBe(false);
	expect(await maybeSendResumeRecap(stubSession, inFlightSession, NOW)).toBe(false);
	expect(calls).toEqual([]);
});
