...
- **Side-effecting actions the persona must declare** — set indicator state, rename a project, name a project — are pi.dev **custom tools**, registered via `customTools` on `createAgentSession` (existing pattern in `sidecar/tools/cairn-tools.ts`).
...
- **Slice-04 (slicing) lands as pi.dev skills, not as new sidecar tools.** `write-prd` and `write-issue` are SKILL.md files under `prompts/skills/`, not `defineTool` entries in `sidecar/tools/cairn-tools.ts`. The existing comment in `sidecar/tools/cairn-tools.ts:18-21` that names them as future *tools* is wrong post-slice-04 and is corrected as part of that slice.
