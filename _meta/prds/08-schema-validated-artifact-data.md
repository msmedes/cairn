...
The Cairn's core user-visible artifacts are currently model-generated HTML documents. That makes the panel depend on agent-authored markup, lets presentation drift into prompts, and forces the app to parse generated documents back into renderable state. The Tasks tab compounds the problem by tracking progress through list position and HTML mutation rather than durable task identity.

The user wants the Cairn and Sub-agents to provide artifact content through schema-validated tool calls, while the app owns the actual presentation for Brief, Plan, and Tasks.

## Solution

The user-visible Project, Plan, and Tasks tabs become dedicated app-rendered surfaces backed by schema-validated JSON artifacts. The Cairn or a Sub-agent supplies structured content through custom tools; the tool validates and persists it; the React app renders it through fixed components. Existing HTML artifact support can be removed rather than migrated.

Each Project also gains a hidden project context file that captures durable facts, terms, constraints, and decisions for the Cairn and Sub-agents. It is created alongside the Brief through an explicit tool call, not as a side effect of artifact creation.
...
3. As the Cairn, I want custom artifact tools for creating and updating the Brief and Plan, so that I can respond to user corrections without raw file edits.
...
9. As the Cairn, I want a narrow task-status update tool, so that routine implementation progress cannot accidentally rewrite the whole checklist.
10. As the Cairn, I want Brief and Plan update tools to require a reason, so that user-visible agreement changes are intentional and auditable.
...
- Update the Cairn persona prompt so raw writes to artifact JSON and project context are forbidden; artifact and context changes happen through tools.
...
- Unit-test Cairn tools with injected active Project state and module fakes where useful, following the existing sidecar tool test pattern.
...
- Rendering or exposing project CONTEXT.md to the user. The Cairn may summarize project context conversationally when useful, but the raw file stays hidden.
