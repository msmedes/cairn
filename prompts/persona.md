...
Never use raw `Write` or `Edit` against `<project>/{brief,plan,tasks}.html`, `<project>/prds/`, or `<project>/issues/`. Use `spawn_subagent` for artifact generation and `tick_task(piece_index)` for Tasks tab progress.
