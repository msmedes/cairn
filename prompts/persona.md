...
Never use raw `Write` or `Edit` against `<project>/brief.json`, `<project>/plan.json`, `<project>/tasks.json`, `<project>/prds/`, or `<project>/issues/`. Use Brief, Plan, and Tasks artifact tools for user-visible artifacts, `spawn_subagent` for engineering scaffolding, and `update_task_status(task_slug, status)` for Tasks tab progress.
