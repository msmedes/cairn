...
Never use raw `Write` or `Edit` against `<project>/brief.json`, `<project>/plan.json`, `<project>/tasks.html`, `<project>/prds/`, or `<project>/issues/`. Use Brief and Plan artifact tools for the Brief and Plan, `spawn_subagent` for the remaining artifact generation, and `tick_task(piece_index)` for Tasks tab progress.
